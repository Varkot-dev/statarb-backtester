...
A pairs-trading backtester with a strictly causal signal engine in **OCaml** and
cointegration research, data handling, and reporting in **Python**.

The design goal is not to produce an impressive Sharpe ratio. It is to produce a
number you can *believe* — which means making lookahead bias structurally
impossible rather than merely claiming it is absent, modelling costs honestly,
and reporting whether a result is distinguishable from luck.

**Every number below is generated by `make backtest` and can be re-derived from
the CSVs the pipeline writes.** Nothing is hand-typed.
...
Interviewers ask this, and the answer is not "correlation is noisy" — it is a
statement about the order of integration of the series.
...
While building this I hit a result that looked like a bug and turned out to be a
real modelling error worth documenting.

**Symptom:** every dataset reported positive total return with a *negative*
Sharpe ratio.
...
| Claim | Verified by |
| --- | --- |
| "OCaml, Python" | `engine/` (11 modules, 2,276 lines) and `python/statarb/` (8 modules) |
...
**Claims this implementation does not fully support:** none. Every element of the
resume line is implemented, tested, and exercised by the pipeline.
