...
type t = {
  ...
  risk_free_rate : float;
      (** Annual, as a decimal (0.04 = 4%). Subtracted from returns before
          computing Sharpe. Stated explicitly because a "Sharpe ratio" quoted
          without its risk-free assumption is not a well-defined number. *)
}
...
let to_string (c : t) : string =
  Printf.sprintf
    "hedge_window=%d zscore_window=%d entry=%.2f exit=%.2f stop=%.2f \
     ...
     frac=%.3f bars_per_year=%.1f rf=%.4f"
